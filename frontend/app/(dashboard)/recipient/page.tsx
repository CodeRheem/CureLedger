...
export default function RecipientDashboard() {
  ...
  useEffect(() => {
    async function fetchCampaigns() {
      try {
        ...
        setMyCampaigns(
          (data.campaigns || []).filter((c: any) => c.status !== "rejected"),
        );
      } catch (err: any) {
        console.error("Failed to fetch campaigns:", err);

        // ✅ 404 just means the user has no campaigns yet — not a real error
  ...
  const totalRaised = myCampaigns.reduce(
    (sum, c) => sum + (c.amountRaised || c.raisedAmount),
    0,
  );
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        ...
        <StatsCard
          label="Total Target"
          value={`₦${(totalTarget / 1_000_000).toFixed(1)}M`}
        />
        <StatsCard
          label="Total Donors"
          value={hasActiveCampaign ? myCampaigns[0].donorCount : 0}
        />
      </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {campaign.medicalNeed || campaign.condition}
                        </p>
                      </div>
                      <Badge
                        className={
                    ...
                    <div className="flex justify-between text-sm mb-4">
                      <span className="text-muted-foreground">
                        ₦{(raised / 1_000_000).toFixed(1)}M of ₦
                        {(campaign.targetAmount / 1_000_000).toFixed(1)}M
                      </span>
